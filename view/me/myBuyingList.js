...
$("#myBuyingList").on("tap", ".tap", function() {
	var id = $(this).data("id");
	...
	window.location.href = "../../view/buying/buyingDetail.html" + "?id=" + id + "&departureCity=" + departureCity + "&departureCountry=" + departureCountry +
...
$("#showAddBtnBuying").on("tap", "img", function(event) {
	...
	myBuyingList.service.showPush();

});

$("#pushBuyingBtn").on("tap", "img", function() {
	myBuyingList.service.pushBuying();
});

$("#pushTakingBtn").on("tap", "img", function() {
	myBuyingList.service.pushTaking();
});

$("#closeBtn").on("tap", "img", function() {
	myBuyingList.service.closePush();
});
	...
	pullRefresh: {
		...
		up: {
			...
			callback: function() {
				...
				myBuyingList.service.doQuery(pageNo, pageSize, QUERY_MODE_UP);
			}
		},
	}
});
myBuyingList = {
	...
	event: function() {
		...
		$("#showMessge").on("click", myBuyingList.service.showMessage);
	},
	...
	service: {
				...
				if(data.status == AJAX_SECCUSS) {
					...
					if(type === QUERY_MODE_UP) {
						...
						if(pageNo > data.result.pagination.total / pageSize) { 
							...
							myBuyingList.service.doDraw($list, data.result.data);
						}
					}
				}
			});
		...
		doDraw: function(list, rows) {
			// 加载模板
			mui.get("myBuyingList-tmpl.html", function(data) {
